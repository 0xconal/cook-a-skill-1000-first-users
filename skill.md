...
Use web search at these three specific points in the pipeline. Do not search at other points.

**Point 1 — Output 0 (Start Here):** Before finalizing the top-priority Week 1 search query, run it.
→ If real posts found: include one paraphrased example under that action item.
→ Format: `→ Live example found: "[paraphrased post title]" — [platform], [date], [engagement]`
→ If nothing found: adjust the query or swap to the next highest-signal action as #1 priority.

**Point 2 — Output 1 (Community Map):** For each candidate community, search to verify before listing.
→ Reddit: search for posts in the last 7 days. Check self-promo rules in sidebar/wiki.
→ Slack: search for a public join link. Only list if a joinable link is found.
→ X: confirm account or hashtag has posts in the last 30 days.
→ HN: run the query on hn.algolia.com. Only include if relevant threads exist (last 3 months).
→ Mark each entry: ✅ Verified (searched and confirmed) or ⚠️ Estimated (AI knowledge, could not confirm).

**Point 3 — Output 2 (Search Signals):** For each signal category, run the top Reddit or X search query.
→ Replace the invented "Real example" with 1–2 actual posts found (paraphrased — do not reproduce verbatim).
→ Include: paraphrased post title/content, date, platform, engagement signal.
→ If no posts found for a signal: flag it — `⚠️ Low current volume — re-run this query in 2–3 weeks.`

**What web search does NOT do:**
- Post, reply, or DM on your behalf
- Monitor communities in real-time
- Change the human-execution principle

**If web search is unavailable:** Generate from AI knowledge, mark all communities ⚠️ Estimated, and keep the verification note instructing the founder to check manually.
...
**Web search step:** Before outputting the action list, run the search query for the top-priority action (#1). If real posts are found, annotate that action with one paraphrased live example. If no results, revise the query or promote action #2 to the top slot.
...
1. [Platform] → Search: "[exact query with filters]"
   ...
   → Time needed: ~[X] min
   → Live example found: "[paraphrased post title]" — [platform], [date], [engagement] *(only if web search ran)*
...
**Web search step:** For each candidate subreddit, search for posts from the last 7 days before listing. Confirm activity and self-promo rules. Mark each: ✅ Verified (active, rules checked) or ⚠️ Estimated (AI knowledge, unconfirmed).

For each subreddit:
- **Name** (r/xxx with link)
- **Status**: ✅ Verified or ⚠️ Estimated
- **Est. size**
...
**Web search step:** Search for a public join link for each candidate workspace. Only list workspaces where a public joinable link is found. Mark: ✅ Verified (join link found) or ⚠️ Estimated (no public link confirmed).
...
**Web search step:** For each account or hashtag, confirm activity in the last 30 days. Remove or mark ⚠️ any that have gone quiet.
...
**Web search step:** Run each proposed Ask HN search query on hn.algolia.com. Only include queries that return at least one relevant thread in the last 3 months. Mark: ✅ Verified (live threads found) or ⚠️ Estimated.
...
**Web search step:** For each signal category, run the top Reddit or X search query before finalizing. Replace the "Real example" field with an actual post found (paraphrased — do not reproduce verbatim). If no posts are found for a signal, flag it as low-volume instead of inventing an example.
...
Real example (found via search): [paraphrased post — actual post found, not invented]
→ Source: [platform] | [date] | [engagement: upvotes / replies / likes]
→ If no live example found: ⚠️ Low current volume — re-run this query in 2–3 weeks
