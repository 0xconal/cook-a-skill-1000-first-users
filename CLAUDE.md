...
### 9. Check-in Mode is a separate mode — not a follow-up prompt

**Problem:** After the founder has the playbook and does 20 interactions, the natural next step is "what do I change?" If they paste their data back in Playbook Mode, the AI might try to regenerate the whole playbook — the opposite of what's needed.

**Decision:** Check-in Mode is its own pipeline. Input: 6 specific data points (replies sent, upvotes, DMs sent, response rate, best platform, unexpected signals). Output: targeted adjustments only — reprioritize signals, adjust tone if replies got downvoted, narrow ICP if DM response rate is below 10%, retire what isn't working.

**Rule:** Check-in output must never regenerate the full playbook. It produces only the delta — what changes and why. A founder who already has the playbook needs a surgeon, not a new operation.

**Why this closes the loop:** Distribution is a habit, not an event. Without a feedback mechanism, founders do it for one week then stop. Check-in Mode gives the skill a second act — it becomes more useful after real usage, not less.
