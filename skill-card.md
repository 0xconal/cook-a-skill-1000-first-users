...
| **Web app** | Next.js wrapper — run the skill without pasting into Claude Projects |
...
| **Check-in Mode** | After ~20 interactions — report what happened | Paste your 6 data points |
...
## Output (Check-in Mode)

Report back after ~20 interactions. Get back:

- **Performance summary** — your numbers parsed and interpreted
- **What's working** — signals and platforms to double down on
- **What's not** — what to retire and why
- **Adjusted signal priorities** — updated ranked list for the next 2 weeks
- **Tone fix** — only if replies were downvoted (specific rule + corrected version)
- **ICP refinement** — only if DM response rate is below 10%
- **New signal** — only if an unexpected pattern appeared
- **Next 5 actions** — same format as Start Here, calibrated to Week 2+
...
| **Check-in Mode** | A third mode that closes the loop. Report back after 20 interactions — get targeted adjustments, not a new playbook. |
